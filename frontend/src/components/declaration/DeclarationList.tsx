...
import { salesApi } from '../../services/api'; // Assurez-vous d'importer correctement votre API

// Types
interface Declaration {
  _id: string; // Assurez-vous que c'est bien un ObjectId sous forme de chaîne hexadécimale de 24 caractères
  date: string;
  amount: number;
  payment: number;
  isPaid: boolean;
...
export function DeclarationSection({
  ...
  const handleDeclare = async (declaration: Declaration) => {
    try {
      // Log the ID and updates before calling the API
      console.log(`Attempting to update declaration with id: ${declaration._id}`);
      console.log('Updates:', { decStatus: '0.1.0' });

      // Call the API to update decStatus to "0.1.0"
      await salesApi.updateSale(declaration._id, { decStatus: '0.1.0' });
